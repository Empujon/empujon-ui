 ...
 * component set `Emociones acostado`, hoy solo variante `Emoción=Cansadx`).
 *
 ...
 * lib). Selected = fondo naranja + texto negro-900; el ícono también invierte sus
 * colores en Selected (ver `IconCansancioSeleccionado` en `emotionIcons.tsx`) — como
 * el ícono es intercambiable (`icon`), es responsabilidad de quien lo consume pasar
 * la versión coloreada correcta según `selected`.
 */
...
export function EmotionCard({ icon, label, selected = false, onClick, className }: EmotionCardProps) {
  return (
    ...
    >
      <span className="relative size-[110px] shrink-0" aria-hidden="true">
        {icon}
      </span>
      <span
        className={cn(
